...
func TestApplyHeaders_Precedence(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			requestHeaders: map[string]string{
				"x-apikey": "request-key",
			},
			checkHeader:   "x-apikey",
			expectedValue: "request-key",
			description:   "Per-request API key should override global",
		},
		{
			name: "global only",
			globalHeaders: map[string]string{
				"x-apikey": "global-api-key",
			},
			requestHeaders: map[string]string{},
			checkHeader:    "x-apikey",
...
func TestApplyHeaders_CommonHeaders(t *testing.T) {
	...
	expectedHeaders := map[string]string{
		"User-Agent":   "VirusTotalSDK/1.0",
		"Accept":       "application/json",
		"x-apikey":     "request-key", // Should be overridden
		"X-Request-ID": "12345",
		"Content-Type": "application/json",
	}
...
func TestApplyHeaders_VirusTotalSpecific(t *testing.T) {
	// Test VirusTotal-specific headers
	client := &Client{
		client: resty.New(),
		globalHeaders: map[string]string{
			APIKeyHeader: "test-api-key-123",
		},
	}
	...
	if got := req.Header.Get(APIKeyHeader); got != "test-api-key-123" {
		t.Errorf("API key header = %q, want %q", got, "test-api-key-123")
	}
}
