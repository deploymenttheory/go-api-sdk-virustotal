...
The VirusTotal SDK uses [zap](https://github.com/uber-go/zap) for high-performance structured logging. Structured logs use key-value pairs instead of formatted strings, making logs easier to parse, search, and analyze.
...
func main() {
    ...
    if err != nil {
        logger.Error("Failed to get file report",
            zap.Error(err),
    ...
    logger.Info("Successfully retrieved file report",
        zap.String("file_name", result.Data.Attributes.MeaningfulName),
        zap.Int("malicious_count", result.Data.Attributes.LastAnalysisStats.Malicious),
    )
}
```
...
```json
{
  "level": "info",
  "ts": 1704891234.567,
  "caller": "client/client.go:71",
  "msg": "VirusTotal API client created",
...
```go
func processFile(logger *zap.Logger, filesService *files.Service, hash string) error {
    logger.Info("Starting file processing",
        zap.String("hash", hash),
    )

    result, resp, err := filesService.GetFileReport(context.Background(), hash)
    if err != nil {
        logger.Error("File report failed",
            zap.String("hash", hash),
            zap.Error(err),
            zap.Int("status_code", resp.StatusCode),
        )
    ...
    logger.Info("File processing complete",
        zap.String("hash", hash),
        zap.String("file_name", result.Data.Attributes.MeaningfulName),
        zap.Int("malicious", result.Data.Attributes.LastAnalysisStats.Malicious),
...
if err != nil {
    logger.Error("Operation failed",
        zap.Error(err),
        zap.String("operation", "get_file_report"),
        zap.String("hash", hash),
...
config := zap.NewProductionConfig()
config.OutputPaths = []string{
    "/var/log/app/virustotal.log", // Splunk monitors this file
}
...
vtClient, _ := client.NewClient(apiKey, client.WithLogger(logger))
```
...
config.OutputPaths = []string{
    "/var/log/app/virustotal.json", // Datadog agent watches this
}
...
func TestMyFunction(t *testing.T) {
    ...
    vtClient, _ := client.NewClient("test-key", client.WithLogger(logger))
...
func TestLogging(t *testing.T) {
    ...
    assert.Equal(t, "VirusTotal API client created", logs[0].Message)
}
```
